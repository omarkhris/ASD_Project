...
public class BankFrm extends JFrame {
	...
	public BankFrm() {
		...
		bankService = new AccountServiceImpl();
...
//        rowdata = new Object[8];
	...
	void JButtonPerAC_actionPerformed(ActionEvent event) {
		...
		*/
		...
		if (newaccount) {
			...
			// add row to table
			rowdata[0] = newCustomer.getAccounts().get(0).getAccountNumber();
			rowdata[1] = newCustomer.getName();
			rowdata[2] = newCustomer.getAddress();
			rowdata[3] = "Personal";
			rowdata[4] = newCustomer.getAccounts().get(0).getAccountType();
			rowdata[5] = newCustomer.getAccounts().get(0).getBalance();
	...
	void JButtonDeposit_actionPerformed(ActionEvent event) {
		...
		if (selection >= 0) {
			...
			// compute new amount
			model.setValueAt(String.valueOf(currentBalance), selection, 5);
		}
