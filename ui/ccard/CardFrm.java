...
import edu.mum.cs.cs525.labs.exercises.project.console.framework.AccountService;
import edu.mum.cs.cs525.labs.exercises.project.console.framework.AccountServiceImpl;
...
public class CardFrm extends JFrame {
	...
	public AccountService getBankService() {
...
//        rowdata = new Object[8];
	...
	void JButtonNewCCAC_actionPerformed(java.awt.event.ActionEvent event) {
		...
		*/
		...
		if (newaccount) {
			...
			rowdata[1] = newCustomer.getAccounts().get(0).getAccountNumber();
			rowdata[2] = expdate;
			rowdata[3] = newCustomer.getAccounts().get(0).getAccountType();
			rowdata[4] = newCustomer.getAccounts().get(0).getBalance();
	...
	void JButtonDeposit_actionPerformed(java.awt.event.ActionEvent event) {
		...
		if (selection >= 0) {
			String accnbr = (String) model.getValueAt(selection, 1);
			System.out.println(accnbr);

			//Show the dialog for adding deposit amount for the current mane
			JDialog_Deposit dep = new JDialog_Deposit(thisframe, accnbr);
			dep.setBounds(430, 15, 275, 140);
			dep.show();

			// compute new amount

			model.setValueAt(String.valueOf(currentBalance), selection, 4);
		}
